        ...
        <td>
            <table>
                <tr>
                    ...
                    <th class="column_date">Дата изменения</th>
                ...
                <?php
                    foreach($this->sections as $section) {
                        if (isset($this->checkpoints[$student['user_id']][$section['section_id']])) {
                            ...
                            $status = 'доступен';
                            $action = $this->_links->get('teacher.remove_checkpoint', array('student_id' => $student['user_id'], 'section_id' => $section['section_id']));
                            $action_title = 'отменить доступ';
                        } else {
                            $created = '&mdash';
                            $status = 'не доступен';
                            $action = $this->_links->get('teacher.set_checkpoint', array('student_id' => $student['user_id'], 'section_id' => $section['section_id']));
...
<?php } else { ?>
<p>Этот курс не изучается ни одним слушателем.</p>
<?php } ?>